...
int main( int argc, char **argv) {
	int status, limit, i, pspace, pos;
	if ( argc != 8) { printf( "bg.udp.tx [1 rip][2 rport][3 psize][4 probesize][5 pspacestart][6 psapacestep][7 pspaceend]\n"); exit( 1); }
	char *rip = argv [ 1];
	int rport = atoi( argv[ 2]);
	int psize = atoi( argv[ 3]);
	int probesize = atoi( argv[ 4]);	// usec
	int pspacestart = atoi( argv[ 5]);
	int pspacestep = atoi( argv[ 6]);
	int pspaceend = atoi( argv[ 7]);
	...
	char msg[ 128];
	for ( pspace = pspacestart; pspace >= pspaceend; pspace -= pspacestep) {
		sleep( 1); double before = getime(); double now = before;
